...
2. **Quality Gate Verifier:** Kiểm tra giới hạn số phần trị, kiểm tra sự tồn tại của ID đối soát với facts/DAL, đối soát tài chính bằng Decimal arithmetic, và kiểm tra tính nhất quán giữa `case_status` và `recommended_refund_brl`.
